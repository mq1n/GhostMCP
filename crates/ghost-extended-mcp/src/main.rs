...
//!
//! Entry point for the ghost-extended-mcp binary.
//! Supports stdio and TCP transports.

use clap::Parser;
use ghost_extended_mcp::{create_server_with_tools, validate_registry, PORT};
use ghost_mcp_common::{error::Result, Transport};
use tracing::info;
use tracing_subscriber::{fmt, prelude::*, EnvFilter};

#[derive(Parser, Debug)]
#[command(name = "ghost-extended-mcp")]
#[command(about = "Ghost-MCP Extended Capabilities Server - Port 13343")]
#[command(version)]
struct Args {
...
async fn main() -> Result<()> {
    ...
    if args.validate_registry {
        info!("Validating ghost-extended-mcp registry...");
        let registry = ghost_extended_mcp::create_registry()?;
    ...
    if args.show_tools {
        let registry = ghost_extended_mcp::create_registry()?;
        println!("ghost-extended-mcp tools ({}):", registry.len());
    ...
    info!(
        "Starting ghost-extended-mcp on {:?}",
        match transport {
